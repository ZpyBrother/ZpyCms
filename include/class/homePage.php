...
class zPage{
	...
	function __construct($sql = null,$count_field = "*",$db,$use_ado_recordcount = false,$page_num=null)
	{
		...
		$this->element["previous_pages"] = "...";
		$this->element["next_pages"] = "...";
		$this->element["per_page_select_number"] = array(9999999=>_lang_all,5=>"5",10=>"10",20=>"20",50=>"50",100=>"100");
		
		$this->page_html_schema["body"] = '<div class="b_yebiao"><ul>%s</ul></div>';
		$this->page_html_schema["count_all"] = '<li class="b_yebiao_yeshu"><a>%s</a></li>';
		$this->page_html_schema["page_button"] = '<li><a href="%s">%s</a></li>';
		$this->page_html_schema["edge_button_on"] = '<li><a href="%s">%s</a></li>';
		$this->page_html_schema["edge_button_off"] = '<li><a  href="%s">%s</a></li>';
		$this->page_html_schema["on_this_page"] = '<li class="b_yeb_on"><a>%d</a></li>';
	...
	function GetPageButtonHref($page)
	{
		if ($page >0){
			$get=$this->GVar->fget;
			unset($get['page']);
			unset($get['m'],$get['o'],$get['t'],$get['cid']);
			foreach ($get as $k=>$v){
				$url[]=$k."=".$v;
			}
			$str="?page=$page&".implode("&", $url);
		}
		return $str;
	}
	//取得分頁條HTML
	function getPageMenu()
	{
	   ...
	   if (!$this->GVar->fget['page']) $this->GVar->fget['page']=1;
	    
		$button["count_all"] = sprintf($this->page_html_schema["count_all"],$this->GVar->fget['page']."/".ceil($this->total_record/$this->this_lots_per_page));
		//第一頁按鈕
		if ($this->page==1)
		{
